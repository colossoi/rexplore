...
mod tests {
    ...
    #[test]
    fn test_enum_from_real_json() {
        // Load and deserialize real JSON structure
        let json_str = include_str!("../test_enum.json");
        let crate_data: rustdoc_types::Crate =
            serde_json::from_str(json_str).expect("Failed to parse JSON");

        let options = Options::default();
        let result = public_api_in_crate(&crate_data, options);

        // Should have: root module, enum, variant1, variant2
        assert_eq!(
            result.items.len(),
            4,
            "Expected 4 items but got {}. Items:\n{}",
            result.items.len(),
            result
                .items
                .iter()
        ...
        let items_str: Vec<String> = result.items.iter().map(|i| i.to_string()).collect();

        // Should have module
        assert!(
            items_str.iter().any(|s| s.contains("pub mod test_crate")),
        ...
        assert!(
            items_str
                .iter()
                .any(|s| s.contains("pub enum test_crate::Error")),
        ...
        assert!(
            ...
            "Should have Syntax variant"
        );
        assert!(
            items_str
                .iter()
                .any(|s| s.contains("Error::CompiledTooBig")),
            "Should have CompiledTooBig variant"
        );
    }
}
